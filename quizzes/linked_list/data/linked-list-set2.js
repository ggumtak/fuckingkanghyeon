/**
 * Linked List Quiz - Set 2 (V2 Format)
 * Variables, conditions, and pointer assignments focus
 */
const linkedListSet2 = {
    setId: "linked-list-set-2",
    questions: [
        {
            id: "q1_full_code_variables",
            type: "code-fill",
            prompt: "다음 전체 코드에서 변수 선언, 조건식, 포인터 할당 부분의 빈칸을 채워 완성하세요.",
            language: "python",
            code: `## 클래스와 함수 선언 부분 ##
class Node():
    def __init__(self):
...
if __name__ == "__main__":
    ...
    while select != 9:
        ...
        if select == 1:
            ...
            blanks: [
                { index: 1, answer: "print(current.data, end=' ')" },
                { index: 2, answer: "current is not None" },
                { index: 3, answer: "node = Node()" },
                { index: 4, answer: "global head" },
                { index: 5, answer: "head.data == findData" },
                { index: 6, answer: "current.data == findData" },
                { index: 7, answer: "newNode.data = data" },
                { index: 8, answer: "currentIndex += 1" },
                { index: 9, answer: "current.link = newNode" },
                { index: 10, answer: "del(current)" },
                { index: 11, answer: "del(current)" },
                { index: 12, answer: "indexes = []" },
                { index: 13, answer: "current = current.link" },
                { index: 14, answer: "data_list = get_list_data()" },
                { index: 15, answer: "FileNotFoundError" },
                { index: 16, answer: "head = None" },
                { index: 17, answer: "appendNode(data)" },
                { index: 18, answer: "findData = input(\"어느 데이터 앞에 삽입할까요? (찾을 데이터): \")" },
                { index: 19, answer: "print(\"인덱스는 숫자로 입력해야 합니다.\")" },
                { index: 20, answer: "confirm.lower() == 'y'" },
                { index: 21, answer: "print(f\"'{data}' 데이터는 리스트에 없습니다.\")" }
            ]
        }
    ]
};

if (typeof window !== 'undefined') window.linkedListSet2 = linkedListSet2;
